...
CSRCS 			+= $(wildcard ui/*.c)
CSRCS 			+= $(wildcard ui/**/*.c)
CSRCS 			+= $(wildcard app/**/*.c)
CSRCS 			+= $(wildcard src/*.c)
CSRCS 			+= $(wildcard src/util/*.c)
...
$(BUILD_OBJ_DIR)/%.o: %.c
	@mkdir -p $(dir $@)
	@$(CC)  $(CFLAGS) -c $< -o $@ -pthread $(LIBS) $(HEADER_PATH)/util/ $(HEADER_PATH)/base/ $(HEADER_PATH)/agentlite $(HEADER_PATH)/
	@echo "CC $<"
...
default: $(TARGET)
	...
	$(CC) -o $(BUILD_BIN_DIR)/$(BIN) $(TARGET) $(LDFLAGS) -pthread $(LIBS) $(HEADER_PATH)/util/ $(HEADER_PATH)/base/ $(HEADER_PATH)/agentlite $(HEADER_PATH)/
